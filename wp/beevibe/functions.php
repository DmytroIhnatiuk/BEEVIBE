...
function beevibe_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'beevibe' ),
		)
	);
 ...
 */
function beevibe_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'beevibe' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'beevibe' ),
...
add_action( 'widgets_init', 'beevibe_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function beevibe_scripts() {
	wp_enqueue_style( 'beevibe-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'beevibe-style', 'rtl', 'replace' );

	wp_enqueue_script( 'beevibe-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'beevibe_scripts' );
